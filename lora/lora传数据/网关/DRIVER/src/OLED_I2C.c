...
void OLED_OFF(void)
{
  ...
  * @param  x,y : 起始点坐标(x:0~127, y:0~7);   修改 x  y  就修改坐标了    y  = 0 ，就是第一行，1 就是第二行
	*					ch[] :- 要显示的字符串; x就是列
...
void OLED_Refresh_Gram(void)
{
	...
	for(i=0;i<8;i++)  
	{  
		I2C_WriteByte (0xb0+i,0);    //设置页地址（0~7）
		I2C_WriteByte (0x00,0);      //设置显示位置—列低地址
		I2C_WriteByte (0x10,0);      //设置显示位置—列高地址   
		for(n=0;n<128;n++)I2C_WriteByte(OLED_GRAM[n][i],1); //更新显存到LCD	
	}   
}
