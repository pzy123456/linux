...
static void SendCmd(char* cmd, char* result, int timeOut)
{
    while(1)
    {
...
/****wifi模块与Tlink平台数据对接配置 */
    SendCmd(AT, "OK", Delay);		//模块有效性检查
		printf("1.AT\r\n");
    SendCmd(CWMODE, "OK", Delay);	//模块工作模式
    //SendCmd(wifi_RST, "OK", Delay);		//模块重置
		printf("2.AT+CWMODE=1\r\n");
	  delay_ms(2000);								//等待模块复位完成
	
    SendCmd(CWJAP, "OK", 2000);		//配置需要连接的WIFI热点SSID和密码  CIFSR
	  printf("3.AT+CJWAP\r\n");
    SendCmd(CIPSTART, "OK", Delay);	//TCP连接
		printf("4.AT+CIPSTART\r\n");
    SendCmd(CIPMODE1, "OK", 2000);	//配置透传模式
	  printf("5.AT+CIPMODE\r\n");
